...
const char *Logger::level_names[] = {"DEBUG", "INFO", "WARN", "ERROR",
                                     "CRITICAL"};
...
void Logger::log(LogLevel level, const char *file, int line, const char *fmt,
  ...
  if (_logger_suppressed > 0 && _logger_tokens >= 2.0f) {
    ...
    Serial.printf("[%6lu] [%-8s] [Heap:%6u] %s (%s:%d)\n", millis() / 1000,
                  ...
                  "[logger] Suppressed messages due to rate limiting", "logger",
                  0);
    _logger_tokens -= 1.0f;
    Serial.printf("[%6lu] [%-8s] [Heap:%6u] Suppressed count: %u (%s:%d)\n",
                  millis() / 1000, "INFO", ESP.getFreeHeap(),
                  _logger_suppressed, "logger", 0);
    _logger_suppressed = 0;
  }
  ...
  _logger_tokens -= 1.0f;
  Serial.printf("[%6lu] [%-8s] [Heap:%6u] %s (%s:%d)\n", millis() / 1000,
                level_names[level], ESP.getFreeHeap(), buffer, filename, line);
}
