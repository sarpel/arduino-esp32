...
bool I2SAudio::initialize() {
  ...
  i2s_config_t i2s_config = {
      ...
      .communication_format =
          I2S_COMM_FORMAT_STAND_I2S, // Use non-deprecated constant
...
bool I2SAudio::readData(uint8_t *buffer, size_t buffer_size,
                        size_t *bytes_read) {
  if (!is_initialized) {
  ...
  esp_err_t result =
      i2s_read(I2S_PORT, buffer, buffer_size, bytes_read, pdMS_TO_TICKS(1000));
  ...
  if (result != ESP_OK) {
    ...
    if (error_type == I2SErrorType::TRANSIENT) {
      ...
      permanent_errors++;
      LOG_ERROR("I2S read permanent error (%d) - reinitialization recommended",
                result);
    } else {
...
bool I2SAudio::readDataWithRetry(uint8_t *buffer, size_t buffer_size,
                                 size_t *bytes_read, int max_retries) {
  for (int attempt = 0; attempt < max_retries; attempt++) {
    ...
    if (consecutive_errors > MAX_CONSECUTIVE_FAILURES) {
      LOG_CRITICAL(
          "Too many consecutive I2S errors - attempting reinitialization");
      if (reinitialize()) {
...
bool I2SAudio::healthCheck() {
  ...
  if (consecutive_errors > (MAX_CONSECUTIVE_FAILURES / 2)) {
    LOG_WARN("I2S health check: %d consecutive errors detected",
             consecutive_errors);
    return false;
  }
...
uint32_t I2SAudio::getErrorCount() { return total_errors; }

uint32_t I2SAudio::getTransientErrorCount() { return transient_errors; }

uint32_t I2SAudio::getPermanentErrorCount() { return permanent_errors; }
