...
static inline unsigned long apply_jitter(unsigned long base_ms) {
  ...
  // Calculate jitter range with safety check for negative values and overflow
  // protection
  int32_t jitter_range =
      (int32_t)((uint64_t)base_ms * SERVER_BACKOFF_JITTER_PCT / 100);
  if (jitter_range < 0) {
...
ExponentialBackoff::ExponentialBackoff(unsigned long min_ms,
                                       unsigned long max_ms)
    : min_delay(min_ms), max_delay(max_ms), current_delay(min_ms),
...
NonBlockingTimer NetworkManager::server_retry_timer(SERVER_RECONNECT_MIN,
                                                    false);
...
// Configure static IP if enabled
  ...
  // Start server retry timer in expired state for immediate first connection
  // attempt This avoids unnecessary 5-second startup delay
...
void NetworkManager::handleWiFiConnection() {
  ...
  if (wifi_retry_count > WIFI_MAX_RETRIES) {
    ...
    LOG_CRITICAL("WiFi connection failed after %d attempts - backing off %lu "
                 "ms (no reboot)",
                 WIFI_MAX_RETRIES, backoff);
...
void NetworkManager::monitorWiFiQuality() {
  ...
  if (rssi < RSSI_WEAK_THRESHOLD) {
    LOG_WARN("Weak WiFi signal: %d dBm - increasing buffer, avoiding forced "
             "disconnect",
             rssi);
...
bool NetworkManager::connectToServer() {
  ...
  LOG_INFO("Attempting to connect to server %s:%d (attempt %d)...", SERVER_HOST,
           SERVER_PORT, server_backoff.getFailureCount() + 1);
  ...
  if (client.connect(SERVER_HOST, SERVER_PORT)) {
    ...
    if (sockfd >= 0) {
      ...
      // This matches server's receiver.py configuration:
      ...
      LOG_DEBUG("TCP socket options configured: TCP_NODELAY=1, keepalive "
                "enabled, send timeout=%dms",
                TCP_WRITE_TIMEOUT);
      LOG_INFO(
          "Audio streaming configured: %d bytes per chunk (%.0fms at 16kHz)",
          TCP_CHUNK_SIZE, (float)TCP_CHUNK_SIZE / 32.0f);
    }
...
WiFiClient &NetworkManager::getClient() { return client; }
...
bool NetworkManager::writeData(const uint8_t *data, size_t length) {
  ...
  if (first_transmission && length > 0) {
    LOG_INFO("Starting audio transmission: first chunk is %u bytes (%.0fms of "
             "audio)",
             (unsigned)length, (float)length / 32.0f);
  ...
  while (total_sent < length) {
    ...
    if (sent == 0) {
      LOG_ERROR("TCP write returned 0 (timeout or error) after %u/%u bytes",
                (unsigned)total_sent, (unsigned)length);
...
uint32_t NetworkManager::getTCPErrorCount() { return tcp_error_count; }
...
void NetworkManager::updateTCPState(TCPConnectionState new_state) {
  if (tcp_state != new_state) {
    ...
    LOG_INFO("TCP state transition: %s → %s", getTCPStateName(old_state),
             getTCPStateName(new_state));
...
void NetworkManager::handleTCPError(const char *error_source) {
  ...
  // ERROR state recovery: The next call to connectToServer() will attempt
  // reconnection with exponential backoff. The ERROR state is a transient state
  // that leads to either:
  // 1. DISCONNECTED (if connection is lost) → next connection attempt resets to
  // CONNECTING
  // 2. CONNECTED (if error is recovered) → normal operation resumes
  // 3. Another ERROR (if connection remains problematic) → exponential backoff
  // continues
  //
  // The system does NOT get stuck in ERROR state due to the polling-based
  // reconnection logic in connectToServer() which continuously attempts to
...
bool NetworkManager::validateConnection() {
  ...
  if (state_says_connected && !is_actually_connected) {
    LOG_WARN(
        "TCP state mismatch: state=CONNECTED but client.connected()=false");
    updateTCPState(TCPConnectionState::DISCONNECTED);
    return false;
  }
  ...
  if (!state_says_connected && is_actually_connected) {
    LOG_WARN(
        "TCP state mismatch: state != CONNECTED but client.connected()=true");
    updateTCPState(TCPConnectionState::CONNECTED);
    return true;
  }
...
uint32_t NetworkManager::getTCPStateChangeCount() { return tcp_state_changes; }
