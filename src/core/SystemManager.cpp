...
bool SystemManager::initialize()
{
    ...
    event_bus->subscribe(SystemEvent::SYSTEM_ERROR,
                         ...
                         { handleSystemEvent(SystemEvent::SYSTEM_ERROR, data); });
    event_bus->subscribe(SystemEvent::MEMORY_CRITICAL,
                         [this](const void *data)
                         { handleHealthEvent(SystemEvent::MEMORY_CRITICAL, data); });
    event_bus->subscribe(SystemEvent::NETWORK_DISCONNECTED,
                         [this](const void *data)
                         { handleNetworkEvent(SystemEvent::NETWORK_DISCONNECTED, data); });
...
void SystemManager::enterSafeMode()
{
    ...
    if (audio_processor)
        audio_processor->setSafeMode(true);
    if (network_manager)
        network_manager->setSafeMode(true);
