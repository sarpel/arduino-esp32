...
size_t AdaptiveBuffer::calculateBufferSize(int32_t rssi) {
  ...
  // Design principle: Weak signal = more packet loss/retransmission = need
  // LARGER buffers Strong signal = reliable transmission = can use smaller
  // buffers to save RAM
  //
  // RSSI to buffer size mapping (inversely proportional to signal strength):
  // Strong signal (-50 to -60):     50% = base_size * 0.5   (reliable, minimal
  // buffering) Good signal (-60 to -70):       75% = base_size * 0.75
  // Acceptable (-70 to -80):       100% = base_size         (normal operation)
  // Weak (-80 to -90):             120% = base_size * 1.2  (needs extra
  // buffering) Very weak (<-90):              150% = base_size * 1.5  (maximum
  // buffering for reliability)
  //
  // Note: We cap at 150% to avoid excessive memory usage on long-term weak
  // signals
...
void AdaptiveBuffer::updateBufferSize(int32_t rssi) {
  ...
  if (new_size != current_buffer_size) {
    int change_pct = ((int)new_size - (int)current_buffer_size) * 100 /
...
uint8_t AdaptiveBuffer::getEfficiencyScore() {
  ...
  //
  // This calculates what percentage of the optimal buffer we're currently
  // using. Higher is better - means we're well-matched to current network
  // conditions.
...
int32_t AdaptiveBuffer::getLastRSSI() { return last_rssi; }

uint32_t AdaptiveBuffer::getAdjustmentCount() { return adjustment_count; }
