...
static uint8_t
    audio_buffer[I2S_BUFFER_SIZE]; // Static buffer to avoid heap fragmentation
...
struct SystemStats {
  ...
  void printStats() {
    ...
    LOG_INFO("Uptime: %lu seconds (%.1f hours)", uptime_sec,
             uptime_sec / 3600.0);
    LOG_INFO("Data sent: %llu bytes (%.2f MB)", total_bytes_sent,
             total_bytes_sent / 1048576.0);
    LOG_INFO("WiFi reconnects: %u", NetworkManager::getWiFiReconnectCount());
    LOG_INFO("Server reconnects: %u",
             NetworkManager::getServerReconnectCount());
...
void onStateChange(SystemState from, SystemState to) {
  LOG_INFO("State transition: %s → %s", systemState.stateToString(from).c_str(),
...
void loop() {
  ...
  case SystemState::CONNECTED: {
    ...
    size_t bytes_read = 0;
    if (I2SAudio::readDataWithRetry(audio_buffer, I2S_BUFFER_SIZE,
                                    &bytes_read)) {
      // Send data to server
