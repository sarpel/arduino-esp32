...
char SerialCommandHandler::command_buffer[SerialCommandHandler::BUFFER_SIZE] = {
    0};
...
void SerialCommandHandler::printStatus() {
  ...
  case TCPConnectionState::CONNECTED:
    ...
    LOG_INFO("TCP Connection uptime: %lu ms",
             NetworkManager::getConnectionUptime());
    break;
  ...
  // System State
  LOG_INFO("System State: %s",
           systemState.stateToString(systemState.getState()).c_str());
...
void SerialCommandHandler::handleStatsCommand() {
  ...
  if (NetworkManager::isTCPConnected()) {
    LOG_INFO("Time Since Last Write: %lu ms",
             NetworkManager::getTimeSinceLastWrite());
    LOG_INFO("Connection Uptime: %lu ms",
             NetworkManager::getConnectionUptime());
  }
...
void SerialCommandHandler::printHealth() {
  ...
  // TCP health
  if (NetworkManager::isTCPConnected()) {
    LOG_INFO("✓ TCP Connected - Uptime: %lu ms",
             NetworkManager::getConnectionUptime());
  } else {
    LOG_ERROR("✗ TCP Not Connected - State: %d",
              (int)NetworkManager::getTCPState());
  }
