...
public:
  ...
  static bool validateAll() {
    ...
    if (all_valid) {
      ...
      LOG_CRITICAL(
          "✗ Configuration validation FAILED - critical settings missing");
    }
...
private:
  ...
  static bool validateWiFiConfig() {
    ...
    if (strlen(WIFI_PASSWORD) == 0) {
      LOG_ERROR(
          "WiFi password is empty - must configure WIFI_PASSWORD in config.h");
      valid = false;
    } else {
  ...
  static bool validateServerConfig() {
    ...
    if (strlen(SERVER_HOST) == 0) {
      LOG_ERROR(
          "Server HOST is empty - must configure SERVER_HOST in config.h");
      valid = false;
    } else {
    ...
    if (SERVER_RECONNECT_MIN <= 0) {
      LOG_WARN("SERVER_RECONNECT_MIN is %u ms - should be > 0",
               SERVER_RECONNECT_MIN);
    } else if (SERVER_RECONNECT_MIN < 1000) {
      LOG_WARN("SERVER_RECONNECT_MIN (%u ms) is very short - minimum "
               "recommended is 1000ms",
               SERVER_RECONNECT_MIN);
    } else {
    ...
    if (SERVER_RECONNECT_MAX <= 0) {
      LOG_WARN("SERVER_RECONNECT_MAX is %u ms - should be > 0",
               SERVER_RECONNECT_MAX);
    } else if (SERVER_RECONNECT_MAX < SERVER_RECONNECT_MIN) {
      LOG_ERROR("SERVER_RECONNECT_MAX (%u ms) cannot be less than "
                "SERVER_RECONNECT_MIN (%u ms)",
  ...
  static bool validateI2SConfig() {
    ...
    if (I2S_SAMPLE_RATE <= 0) {
      ...
      LOG_WARN("I2S_SAMPLE_RATE (%u Hz) outside typical range (8000-48000)",
               I2S_SAMPLE_RATE);
    } else {
    ...
    if (I2S_MAX_READ_RETRIES <= 0) {
      LOG_WARN("I2S_MAX_READ_RETRIES is %u - should be > 0",
  ...
  static bool validateTimingConfig() {
    ...
    if (MEMORY_CHECK_INTERVAL <= 0) {
      LOG_WARN("MEMORY_CHECK_INTERVAL is %u ms - should be > 0",
               MEMORY_CHECK_INTERVAL);
    } else if (MEMORY_CHECK_INTERVAL < 5000) {
      LOG_WARN("MEMORY_CHECK_INTERVAL (%u ms) is very frequent",
               MEMORY_CHECK_INTERVAL);
    } else {
    ...
    if (RSSI_CHECK_INTERVAL <= 0) {
      LOG_WARN("RSSI_CHECK_INTERVAL is %u ms - should be > 0",
               RSSI_CHECK_INTERVAL);
    } else {
    ...
    if (STATS_PRINT_INTERVAL <= 0) {
      LOG_WARN("STATS_PRINT_INTERVAL is %u ms - should be > 0",
  ...
  static bool validateMemoryThresholds() {
    ...
    if (MEMORY_CRITICAL_THRESHOLD <= 0) {
      LOG_ERROR("MEMORY_CRITICAL_THRESHOLD must be > 0, got %u bytes",
                MEMORY_CRITICAL_THRESHOLD);
      valid = false;
    } else {
      LOG_INFO("  ✓ Memory critical threshold: %u bytes",
               MEMORY_CRITICAL_THRESHOLD);
    }

    if (MEMORY_WARN_THRESHOLD <= 0) {
      LOG_ERROR("MEMORY_WARN_THRESHOLD must be > 0, got %u bytes",
    ...
    if (MEMORY_CRITICAL_THRESHOLD >= MEMORY_WARN_THRESHOLD) {
      LOG_ERROR(
          "MEMORY_CRITICAL_THRESHOLD (%u) must be < MEMORY_WARN_THRESHOLD (%u)",
    ...
    if (RSSI_WEAK_THRESHOLD > 0) {
      LOG_WARN(
          "RSSI_WEAK_THRESHOLD (%d) is positive - should be negative dBm value",
          RSSI_WEAK_THRESHOLD);
    } else if (RSSI_WEAK_THRESHOLD > -20) {
      LOG_WARN("RSSI_WEAK_THRESHOLD (%d dBm) is very strong - typical range is "
               "-80 to -50",
               RSSI_WEAK_THRESHOLD);
    } else {
    ...
    if (MAX_CONSECUTIVE_FAILURES <= 0) {
      LOG_WARN("MAX_CONSECUTIVE_FAILURES is %u - should be > 0",
  ...
  static bool validateWatchdogConfig() {
    ...
    if (WATCHDOG_TIMEOUT_SEC <= 0) {
      LOG_ERROR("WATCHDOG_TIMEOUT_SEC must be > 0, got %u seconds",
                WATCHDOG_TIMEOUT_SEC);
      valid = false;
    } else if (WATCHDOG_TIMEOUT_SEC < 30) {
      LOG_WARN(
          "WATCHDOG_TIMEOUT_SEC (%u sec) is short - recommend >= 30 seconds",
          WATCHDOG_TIMEOUT_SEC);
    } else {
    ...
    if (WATCHDOG_TIMEOUT_SEC <= wifi_timeout_sec) {
      LOG_WARN("WATCHDOG_TIMEOUT_SEC (%u) <= WIFI_TIMEOUT (%u sec) - watchdog "
               "may reset during WiFi connection",
    ...
    if (WATCHDOG_TIMEOUT_SEC <= error_delay_sec) {
      LOG_ERROR("WATCHDOG_TIMEOUT_SEC (%u) <= ERROR_RECOVERY_DELAY (%u sec) - "
                "watchdog will reset during error recovery",
    ...
    if (WATCHDOG_TIMEOUT_SEC < (wifi_timeout_sec + 5)) {
      LOG_WARN("WATCHDOG_TIMEOUT_SEC (%u) is close to WIFI_TIMEOUT (%u sec) - "
               "margin may be tight",
               WATCHDOG_TIMEOUT_SEC, wifi_timeout_sec);
    }
