...
private:
    ...
    static bool validateWiFiConfig()
    {
        ...
        {
            LOG_ERROR("WIFI_RETRY_DELAY is 0 - must configure WIFI_RETRY_DELAY in config.h");
            valid = false;
        }
        else
        {
    ...
    static bool validateI2SConfig()
    {
        ...
        else
        {
            ...
            {
                LOG_ERROR("AUDIO_GAIN_DENOMINATOR must be greater than zero (no zero or negative values allowed), got %d", AUDIO_GAIN_DENOMINATOR);
                valid = false;
            }
            else
            {
