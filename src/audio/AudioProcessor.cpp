...
bool AudioProcessor::initialize()
{
    ...
    // Note: Assumes I2S_BUFFER_SIZE is defined in config.h and represents bytes
    // Verify I2S_BUFFER_SIZE is reasonable (expected: 4096 bytes)
    if (I2S_BUFFER_SIZE < 512 || I2S_BUFFER_SIZE > 8192)
    {
        if (logger)
        {
            logger->log(LogLevel::LOG_WARN, "AudioProcessor", __FILE__, __LINE__,
                        "I2S_BUFFER_SIZE (%d) outside expected range [512-8192]", I2S_BUFFER_SIZE);
        }
    }

    processing_buffer_size = I2S_BUFFER_SIZE / 2; // 16-bit samples (2 bytes each)
    ...
    // Initialize audio buffers with minimal sizing for ESP32 constraints
    // Original sizing was processing_buffer_size * 4; reduced to processing_buffer_size * 1 to save memory
