                                           ...
                                           uint8_t *raw_data, size_t &raw_size)
{
    ...
    // Find the actual 'data' chunk (handles LIST, fact, and other chunks)
    size_t audio_data_offset = 0;
    size_t audio_data_size = 0;

    if (!findDataChunk(wav_data, wav_size, audio_data_offset, audio_data_size))
    {
                                 ...
                                 uint32_t sample_rate, uint8_t channels, uint8_t bit_depth)
{
    ...
    // For WAV format, write header with placeholder data size
    if (format == AudioFormatType::WAV)
    {
        WAVHeader header;
        AudioFormatConverter::buildWAVHeader(header, sample_rate, channels, 0);

        if (fwrite(&header, sizeof(WAVHeader), 1, file_handle) != 1)
        {
            fclose(file_handle);
            file_handle = nullptr;
            return false;
...
bool AudioStreamWriter::writeAudioData(const uint8_t *data, size_t size)
{
    ...
    total_bytes += size;
    samples_written += (size / (bit_depth / 8));
...
bool AudioStreamWriter::finalizeFile()
{
    ...
    // For WAV files, update header with actual data size
    if (format == AudioFormatType::WAV)
    {
        // Seek to beginning
        ...
        // Rebuild header with actual data size
        WAVHeader header;
        AudioFormatConverter::buildWAVHeader(header, sample_rate, channels, total_bytes);
