...
# - TCP chunk size: 9600 samples × 2 bytes = 19200 bytes (600ms chunks)
...
# TCP buffer size aligned with firmware (600ms chunks from ESP32)
TCP_CHUNK_SIZE = 19200   # 9600 samples × 2 bytes = 19200 bytes (600ms at 16kHz)
...
def tcp_server():
    ...
    while True:
        try:
            ...
            # Increase receive buffer to handle 600ms chunks efficiently
            # ESP32 sends 19200 bytes every 600ms = ~32KB/sec
            ...
            while True:
                try:
                    # Receive data (aligned with firmware 600ms chunks: 9600 samples × 2 bytes = 19200 bytes)
